...
public class SessionBuilder {
	...
	public ClusterSession build() {
		...
		if (this.keyspace == null) {
			...
			LOGGER.info("Try to connect to Cassandra on '{}:{}' with keyspace '{}'.", this.contactPoint, this.port,
					this.keyspace);
		}
	...
	private void createKeyspaceIfNotExists(final Cluster cluster) {
		if (this.keyspace != null) {
			final Session session = cluster.connect();
			final KeyspaceOptions statement = SchemaBuilder.createKeyspace(this.keyspace).ifNotExists().with()
