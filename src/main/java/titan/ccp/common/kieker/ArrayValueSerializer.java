...
/**
 * An {@link IValueSerializer} that serializes a record into a given {@link Object} array. This
 * array has to be large enough to fit in all values. Otherwise, an {@link IllegalArgumentException}
 * is thrown when the first value should be serialized that does not fit in the array.
 *
  ...
  public void putAny(final Object value) {
    if (this.nextIndex >= this.array.length) {
      throw new IllegalArgumentException(
          "The given array is not large enough to serialize all fields of this record.");
    }
